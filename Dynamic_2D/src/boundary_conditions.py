...
class DynamicBoundaryConditionsManager:
    ...
    def _create_displacement_bcs(self):
        ...
        # 侧面法向约束 (x=0 面 marker 103, x=Lx 面 marker 104, y=0 面 marker 107, y=Ly 面 top)
        # 注意：二维中只有左右边界需要约束 x 方向；底部已固定，顶部自由
    ...
    def _create_pressure_bcs(self):
        ...
        创建压力边界条件：
        - 顶部自由排水边界 (p=0)
        - 侧面静水压力分布 p = ρ_w g (H - y)  (二维) 或 p = ρ_w g (H - z) (三维)
        - 注浆孔压力（当前时间值）
        """
        fdim = self.mesh.topology.dim - 1
    ...
    def update(self, time: float, material_props: Optional[Dict] = None):
        ...
        self._create_concentration_bcs()  # 浓度边界条件
