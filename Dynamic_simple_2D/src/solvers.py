...
class SolverManager:
    ...
    def solve(self, dt, time, solution, solution_prev, boundary_conditions=None, materials=None):
        ...
        # 在 solution.x.array[:] = solution_prev.x.array[:] 之后
        arr = solution.x.array
        ...
        if np.any(np.isinf(arr)):
            print("错误：解向量包含 Inf！")
            return False, 0
        print(f"解向量范围: [{arr.min():.3e}, {arr.max():.3e}], 零值数量: {np.sum(arr==0)}")
        ...
        # 手动组装残差向量
        b = assemble_vector(self.problem.L)
        b.assemble()
        b_array = b.getArray()
        ...
        # 手动组装雅可比矩阵
        A = assemble_matrix(self.problem.a, bcs=bcs)
        A.assemble()
