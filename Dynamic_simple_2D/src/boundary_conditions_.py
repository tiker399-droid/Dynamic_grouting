...
class DynamicBoundaryConditionsManager:
    ...
    def __init__(
        ...
        # 参考压力（用于无量纲化）
        self.P = float(config['materials']['grout']['pressure'])
        self.P0 = 1 / self.P
    ...
    def _create_pressure_evolution_function(self) -> Callable[[float], float]:
        ...
        if self.pressure_mode == PressureEvolutionMode.LINEAR_INCREASE:
            def func(t):
                if t < self.rise_time:
                    return ((self.pressure_max - self.pressure_0) * (t / self.rise_time) + self.pressure_0) * self.P0
                elif t < self.grouting_duration:
                    return self.pressure_max / self.P0
                else:
                    return 0.0
    ...
    def _create_pressure_bcs(self):
        ...
        if 'marker_107' in self.boundary_geometries:
            facets = self.boundary_geometries['marker_107']['facets']
            zero_vec = fem.Function(V_p)
            zero_vec.x.array[:] = 127530.0 * self.P0
        ...
        if self.gdim == 2:
            ...
            '''
            water_pressure_func.interpolate(
                lambda x: self.materials.rho_w * self.gravity_magnitude * (self.foundation_height - x[1]) * self.P0
            )
        else:
            water_pressure_func.interpolate(
                lambda x: self.materials.rho_w * self.gravity_magnitude * (self.foundation_height - x[2]) * self.P0
            )
